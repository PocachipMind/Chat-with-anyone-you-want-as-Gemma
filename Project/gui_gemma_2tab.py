        ...
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    ...
                    # 동기화 함수
                    bot_name.change(first_tab_test.sync_bot_name, inputs=bot_name, outputs=profile_name)
                    
                    # 모델 설정 버튼
                    bot_submit_btn.click(first_tab_test.create_profile, inputs=[profile_image, profile_image_back, profile_name, profile_status], outputs=profile_display)
                    
                    # 상태 변수 생성
                    model_state = gr.State(value="")  # 초기값 설정
                    bot_submit_btn.click(first_tab_test.set_model, inputs=[bot_name, bot_gender, bot_profession, bot_personality], outputs=model_state)  # 상태 변수 업데이트
            ...
            with gr.Column():
                ...
                msg.submit(first_tab_test.respond, [msg, chatbot, model_state], [msg, chatbot])
